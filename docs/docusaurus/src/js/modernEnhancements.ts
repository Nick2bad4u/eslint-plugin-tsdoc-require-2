...
function getRuleNumberPrefix(
    ...
    const separatorIndex = trimmedLabel.search(/\s/v);

    if (separatorIndex === -1) {
        return null;
    }

    const numberToken = trimmedLabel.slice(0, separatorIndex);
    const remainder = trimmedLabel.slice(separatorIndex).trimStart();

    if (!/^\d{2,3}$/v.test(numberToken) || remainder.length === 0) {
        return null;
    }
