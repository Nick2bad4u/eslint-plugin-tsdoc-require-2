...
This rule reports supported declarations and default exports when a TSDoc block exists but does not contain `@class`. It does not create the TSDoc block itself, so pair it with [`tsdoc-require-2/require`](../require.md) when you also want to require the comment.
