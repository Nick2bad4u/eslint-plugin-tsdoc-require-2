...
const getExpectedMajorVersion = (argumentsList) => {
  const expectedFlag = argumentsList.find((argument) => argument.startsWith(EXPECT_FLAG));
...
if (!Number.isInteger(currentMajorVersion)) {
  throw new Error(`Unable to parse ESLint version from \"${currentVersion}\".`);
}

const expectedMajorVersion = getExpectedMajorVersion(process.argv.slice(2));
if (expectedMajorVersion !== null && expectedMajorVersion !== currentMajorVersion) {
  throw new Error(
...
const validResult = await eslint.lintText("/** Documented */ export class HasDoc {}", {
...
if (invalidErrorCount === 0) {
  throw new Error("Compatibility smoke test failed: invalid source did not report any errors.");
}

if (validErrorCount !== 0) {
  throw new Error("Compatibility smoke test failed: valid source produced lint errors.");
}
